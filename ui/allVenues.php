...
if(!isset($_SESSION['loggedIn'])){
  header("Location: http://serenity.ist.rit.edu/~dc6288/EMS/login.php");
}
else if($_SESSION['role'] != "admin"){
  header("Location: http://serenity.ist.rit.edu/~dc6288/EMS/ui/dashboard.php");
}
?>
...
<!--Add venue modal-->
<div class="modal fade" id="addVenueModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalCenterTitle">Add Venue</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="form-group row">
            <label for="inputAddVenueName" class="col-sm-3 col-form-label">Name</label>
            <div class="col-sm-9">
                <input type="text" class="form-control" id="inputAddVenueName" required>
            </div>
        </div>
        <div class="form-group row">
            <label for="inputAddVenueCapacity" class="col-sm-3 col-form-label">No. Allowed</label>
            <div class="col-sm-9">
                <input type="number" class="form-control" id="inputAddVenueCapacity">
            </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary add-venue-modal-btn">Add</button>
      </div>
    </div>
  </div>
</div>
...
<div class="container venue-container">
  ...
  <div class="row">
    <button type="button" class="btn btn-primary btn-block add-venue">Add New Venue</button>
  </div>
  <br/>
...
<script>
  $.ajax({
    ...
    success: function(content){
      ...
      $(".add-venue-modal-btn").click(function(){
        let name = $("#inputAddVenueName").val();
        let capacity = $("#inputAddVenueCapacity").val();
        if(name !== "" && capacity !== ""){
          $.ajax({
            url:'http://serenity.ist.rit.edu/~dc6288/EMS/api/api.php?method=addVenue',
            type: "POST",
            data: {name, capacity},
            success: function(content){
              if(content == 1){
      ...
      $(".save-change").click(function(){
        ...
        if(!(name == currentName && capacity == currentCapacity)){
            $.ajax({
            url:'http://serenity.ist.rit.edu/~dc6288/EMS/api/api.php?method=updateVenue',
            type: "POST",
      ...
      $(".delete-venue" ).click(function() { 
          ...
          $.confirm({
              ...
              buttons: {
                  Confirm: function () {
                    $.ajax({
                        url:'http://serenity.ist.rit.edu/~dc6288/EMS/api/api.php?method=deleteVenue',
                        type: "POST",
                        data: {venueId},
                        success: function(content){
                            if(content == 1){
